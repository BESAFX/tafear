app.controller('watchTasksOperationsCtrl', ['$scope', '$rootScope', '$timeout', '$uibModalInstance', 'PersonService', '$http',
    function ($scope, $rootScope, $timeout, $uibModalInstance, PersonService, $http) {
        ...
        $scope.submit = function () {
            ...
            switch ($scope.action) {
                case 'view':
                    window.open('/report/WatchTasksOperations?' + search.join(""));
                    break;
                case 'download':
                    $http.get('/report/WatchTasksOperations?' + search.join(""), {responseType: 'arraybuffer'}).then(function (response) {
